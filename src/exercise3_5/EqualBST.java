...
/*Programming assignment 3-5-9;
 * In this version one Key can have several Values.*/


public class EqualBST<Key extends Comparable<Key>, Value> {
    private Node root;

    private class Node {
        private Key key;
        private Queue<Value> valQ = new Queue<>();
        private Node left, right;
        private int N;    // 该结点为根的子树中的结点数

        public Node(Key key, Value val, int N) {
            this.key = key;
            this.valQ.enqueue(val);
            this.N = N;
        }
    }
